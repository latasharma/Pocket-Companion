...
export class VoiceService {
  ...
  static addEmotionMarkers(text) {
    ...
    // Normalize existing ellipses and ensure a single brief pause after sentence punctuation
    enhancedText = enhancedText.replace(/\.{3,}/g, '... '); // collapse multiple ellipses
    enhancedText = enhancedText.replace(/([^\.\s])\.(\s+[A-Z]|$)/g, '$1... $2'); // single period -> ellipsis + space
    enhancedText = enhancedText.replace(/([^\.\s])\.$/g, '$1...');
    enhancedText = enhancedText.replace(/!/g, '!... ');
    enhancedText = enhancedText.replace(/\?/g, '?... ');

    // Add emphasis for important words by uppercasing the word (do NOT duplicate it)
    enhancedText = enhancedText.replace(/\b(great|wonderful|amazing|excellent|fantastic)\b/gi, (m) => m.toUpperCase());
    enhancedText = enhancedText.replace(/\b(important|critical|essential|vital)\b/gi, (m) => m.toUpperCase());
    enhancedText = enhancedText.replace(/\b(sorry|apologize|regret)\b/gi, (m) => m.toUpperCase());
    enhancedText = enhancedText.replace(/\b(happy|excited|thrilled|delighted)\b/gi, (m) => m.toUpperCase());

    // Add conversational markers with a brief pause without duplicating words
