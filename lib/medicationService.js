...
async function createMedication(payload = {}) {
  try {
    ...
    // Helper: normalize dose time values (accept ISO datetime or HH:MM or HH:MM:SS) -> "HH:MM:SS"
    function normalizeDoseTime(t) {
      if (!t) return null;
      const s = String(t).trim();
      // ISO timestamp -> HH:MM:SS (UTC)
      if (/^\d{4}-\d{2}-\d{2}T/.test(s)) {
        try {
          return new Date(s).toISOString().slice(11, 19);
        } catch (e) {
          return null;
        }
      }
    ...
    // Normalize dose_times for DB (time[] expects HH:MM:SS)
    const normalizedDoseTimes = Array.isArray(payload.dose_times)
      ? payload.dose_times.map((t) => normalizeDoseTime(t)).filter(Boolean)
      : null;
