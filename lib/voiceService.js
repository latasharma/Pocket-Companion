...
// Safely import AudioModule - it may not be available
      ...
      sound.setOnPlaybackStatusUpdate((status) => {
        try {
          if (status.didJustFinish) {
            ...
            // Clean up file
            FileSystem.deleteAsync(filePath, { idempotent: true }).catch(err => console.error('Error deleting file:', err));
            // Notify that voice has ended
            if (this.onVoiceEnd) {
              try {
                this.onVoiceEnd();
              } catch (err) {
                console.error('Error in onVoiceEnd callback:', err);
              }
            }
          }
        } catch (error) {
