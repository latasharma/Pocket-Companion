...
export class VoiceInputService {
  ...
  static AUDIO_FORMATS = {
    ...
    PRIMARY: {
      android: {
        extension: '.m4a',
        outputFormat: 2, //(MPEG_4) //Audio.RECORDING_OPTION_ANDROID_OUTPUT_FORMAT_MPEG_4,
        audioEncoder: 3, //(AAC) //Audio.RECORDING_OPTION_ANDROID_AUDIO_ENCODER_AAC,
        sampleRate: 16000,
        numberOfChannels: 1,
        bitRate: 64000,
      },
      ios: {
        ...
        audioQuality: 1, //(HIGH) //Audio.RECORDING_OPTION_IOS_AUDIO_QUALITY_HIGH,
        sampleRate: 16000,
        numberOfChannels: 1,
        bitRate: 64000,
  ...
  static async initialize() {
    try {
      // On Android, expo-av's requestPermissionsAsync has been observed to hang in
      // some edge cases. Use a short timeout and fall back to PermissionsAndroid
      // to avoid the initialize() promise never resolving.
      if (Platform.OS === 'android') {
        ...
        if (res && res.status === 'granted') {
          ...
          // Fallback: explicitly request via PermissionsAndroid
        ...
        // Non-Android platforms: use expo-av permission request as before
